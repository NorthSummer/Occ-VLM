...
def eval_model(questions, args):
    ...
    for line in tqdm(questions):
        ...
        for k in video_dict:
            if k == 'box_input' or k == 'world_coords' or k == 'objects':
...
if __name__ == "__main__":
    ...
    for i in range(args.n_gpu):
        # 取第 i 个分片的数据
        sub_questions = questions[i::args.n_gpu]
        print(f"Processing {len(sub_questions)} items for shard {i}...")

        # 直接调用 eval_model 函数（不是 .remote）
        result = eval_model(sub_questions, args)  
