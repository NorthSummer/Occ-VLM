...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        model = LlavaQwenMoeForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=lora_cfg_pretrained, **kwargs)
                    else:
                        model = LlavaQwenMoeForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=lora_cfg_pretrained, **kwargs)
                    ...
                    if overwrite_config is not None:
                        ...
                        lora_cfg_pretrained.vocab_size = 151649
                        lora_cfg_pretrained.use_cache = True
                        lora_cfg_pretrained.tie_word_embeddings = False
                        model = LlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=lora_cfg_pretrained, **kwargs)
                        print('Loading LoRA weights...')
                        from peft import PeftModel
                        model = PeftModel.from_pretrained(model, model_path) # 把 LoRA 贴到 Base 模型上
                        print('Merging LoRA weights...')
                        model = model.merge_and_unload() # 合并
                        model._hf_peft_config_loaded = False # 解决你之前的报错
            else:
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        occ_tower = model.get_occ_tower()
        # image_processor = vision_tower.adapter.model.image_processor
