...
def densify_voxel_coords(occ_coords, voxel_size=0.16, subdivisions=3):
    """
    在每个 voxel 内生成均匀分布的子点
    
    Args:
        occ_coords: (N, 3) voxel 中心坐标
        voxel_size: float, 体素边长
        subdivisions: int, 每个维度划分数量 (总共生成 subdivisions³ 个点)
    
    Returns:
        dense_coords: (N * subdivisions³, 3) 密集化后的坐标
        voxel_indices: (N * subdivisions³,) 每个点对应的原始 voxel 索引
    """
    N = occ_coords.shape[0]
    device = occ_coords.device
    
    # ── 生成 voxel 内的相对偏移 ──────────────────────────────────
    # 例如 subdivisions=2: [-0.04, 0.04] (voxel_size/4 的偏移)
    step = voxel_size / subdivisions
    offset_1d = torch.linspace(
        -voxel_size/2 + step/2,  # 从边界内侧开始
        voxel_size/2 - step/2,
        subdivisions,
        device=device
    )  # (subdivisions,)
    ...
    offset_x, offset_y, offset_z = torch.meshgrid(
        offset_1d, offset_1d, offset_1d, indexing='ij'
    )  # 每个 (subdivisions, subdivisions, subdivisions)
    
    offsets = torch.stack([
        offset_x.reshape(-1),
        offset_y.reshape(-1),
        offset_z.reshape(-1)
    ], dim=1)  # (subdivisions³, 3)
    
    # ── 广播到所有 voxel ─────────────────────────────────────────
    dense_coords = occ_coords[:, None, :] + offsets[None, :, :]  
    # (N, 1, 3) + (1, S³, 3) = (N, S³, 3)
    
    dense_coords = dense_coords.reshape(-1, 3)  # (N*S³, 3)
    
    # ── 记录每个点对应的原始 voxel 索引 ─────────────────────────
    voxel_indices = torch.arange(N, device=device)[:, None].repeat(
        1, subdivisions**3
    ).reshape(-1)  # (N*S³,)
    
    return dense_coords, voxel_indices
...
class LlavaMetaForCausalLM(ABC):
    ...
    def mask_average_coordinate_in_patch(self, world_coords, valid_mask, patch_size=27):
        """
        Args:
            world_coords: (V, H, W, 3)  无效点坐标为 (0, 0, 0)
            valid_mask:   (V, H, W)     bool, True 表示该像素有有效的3D投影
            patch_size:   int
        Returns:
            world_coords_avg: (V, patch_num, patch_num, 3)
            valid_mask_pooled: (V, patch_num, patch_num)  bool, patch内至少有一个有效点则为True
        """
        V, H, W, D = world_coords.size()

        world_coords = world_coords[:, :-6, :-6, :]          # (V, 378, 378, 3)
        valid_mask   = valid_mask[:, :-6, :-6]               # (V, 378, 378)

        world_coords = world_coords.permute(0, 3, 1, 2)      # (V, 3, 378, 378)
        valid_mask_float = valid_mask.float()                 # (V, 378, 378)

        valid_count = torch.nn.functional.avg_pool2d(
            valid_mask_float.unsqueeze(1),                   # (V, 1, 378, 378)
            kernel_size=patch_size,
            stride=patch_size
        ) * (patch_size * patch_size)                        # (V, 1, P, P)  float

        coords_sum = torch.nn.functional.avg_pool2d(
            world_coords,                                    # (V, 3, 378, 378)
            kernel_size=patch_size,
            stride=patch_size
        ) * (patch_size * patch_size)                        # (V, 3, P, P)

        safe_count = valid_count.clamp(min=1.0)              
        world_coords_avg = coords_sum / safe_count           # (V, 3, P, P)

        world_coords_avg = world_coords_avg.permute(0, 2, 3, 1)  # (V, P, P, 3)
        
        # 将 valid_count 转为 bool: patch 内至少有一个有效点
        valid_mask_pooled = (valid_count.squeeze(1) > 0)     # (V, P, P) bool

        return world_coords_avg.to(world_coords.dtype), valid_mask_pooled
    ...
    def encode_images_with_occ(self, images, video_dict=None):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # dense_points_2d, dense_2d_mask = self.get_model().get_occ_tower().project_dense_points(video_dict)
        # dense_points_2d, dense_2d_mask = dense_points_2d.to(input_ids.device), dense_2d_mask.to(input_ids.device)
        # world_coords = dense_points_2d.unsqueeze(0).to(video_dict["world_coords"][0].dtype)
        # coords_mask = dense_2d_mask.unsqueeze(0)
        ...
        if type(images) is list or images.ndim == 5:
            ...
            encoded_occ_features, occ_coords, vfm_last_hidden = self.encode_images_with_occ(concat_images, video_dict)

        dense_occ_pts, _ = densify_voxel_coords(occ_coords)
        ...
        if use_object_proposals:
            ...
            world_coords_ori = video_dict["world_coords"][0]
            dense_points_2d, dense_2d_mask = self.get_model().get_occ_tower().project_dense_points(video_dict, dense_occ_pts)
            dense_points_2d, dense_2d_mask = dense_points_2d.to(input_ids.device), dense_2d_mask.to(input_ids.device)
            world_coords = dense_points_2d.to(video_dict["world_coords"][0].dtype)
            coords_mask = dense_2d_mask
        ...
        if hasattr(self.config, 'world_position_embedding_type') and past_key_values is None:
            ...
            if 'avg' in self.config.world_position_embedding_type:
                ...
                world_coords_mask = [self.mask_average_coordinate_in_patch(coords, mask)[1] for coords, mask in zip(world_coords, coords_mask)]
                world_coords = [self.mask_average_coordinate_in_patch(coords, mask)[0] for coords, mask in zip(world_coords, coords_mask)]
                ...
                for image_idx, image_feature in enumerate(image_features):
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                        ...
                        if mm_newline_position == "grid":
                            ...
                            valid_mask = world_coords_mask[image_idx].flatten(1, 2)
                            valid_mask = valid_mask.flatten(0, 1)                            
                            new_image_features.append(image_feature.flatten(0, 1)[valid_mask])
                        else:
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if hasattr(self.config, "coord_token_ids") and (use_sin3d_pe or use_mlp_pe):
                ...
                if query_coord_tokens.sum() != 0:
                    ...
                    world_pos_emb = self.get_model().world_position_embedding(box_input.unsqueeze(0).detach())[:, 0]
                    cur_input_embeds = cur_input_embeds.clone()
                    cur_input_embeds[query_coord_tokens] = cur_input_embeds[query_coord_tokens] + world_pos_emb
