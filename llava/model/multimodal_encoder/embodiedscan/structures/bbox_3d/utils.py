...
from llava.model.multimodal_encoder.embodiedscan.utils.array_converter import array_converter
...
def batch_points_cam2img(points_3d, proj_mat, with_depth=False):
    ...
    """ 
    # points_3d_shape torch.Size([20, 25600, 3])
    points_shape = list(points_3d.shape)
    points_shape[-1] = 1
    ...
    # previous implementation use new_zeros, new_one yields better results
    points_4 = torch.cat([points_3d, points_3d.new_ones(points_shape)], dim=-1).to(proj_mat.dtype)

    # torch.Size([20, 25600, 4])
    # torch.Size([20, 4, 4])
