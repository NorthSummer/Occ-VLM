...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    overwrite_config = {}
    overwrite_config['vocab_size'] = 151649
...
def train(attn_implementation=None):
    ...
    # try:
    #     rank = int(training_args.local_rank) if hasattr(training_args, "local_rank") else (torch.distributed.get_rank() if torch.distributed.is_initialized() else 0)
    # except:
    #     rank = 0
    # def show_param_shapes(mod, prefix=""):
    #     for n, p in mod.named_parameters():
    #         # 尝试获取 deepspeed/ZeRO 的属性（若存在）
    #         ds_status = getattr(p, "ds_status", None)
    #         ds_numel = getattr(p, "ds_numel", None)
    #         shape = tuple(p.shape)
    #         print(f"[RANK {rank}] {prefix}{n}: shape={shape}, numel_ds={ds_numel}, ds_status={ds_status}")
    # if hasattr(model, "get_occ_tower"):
    #     occ = model.get_occ_tower()
    #     show_param_shapes(occ, prefix="occ.")
    # else:
    #     print(f"[RANK {rank}] model has no occ_tower")
    ...
    #         target_modules=["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"], # find_all_linear_names(model),
    #         lora_dropout=training_args.lora_dropout,
    #         bias=training_args.lora_bias,
    #         task_type="CAUSAL_LM",
    #         modules_to_save=["embed_tokens", "lm_head", "ground_head"],
    #     )
    ...
    if model_args.ground_head_type:
        from llava.constants import GROUND_TOKEN
    ...
    if model_args.vision_tower is not None:
        ...
        if training_args.lora_enable:
            ...
            if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
                ...
                # Set the entire model to not require gradients by dOefault
                model.requires_grad_(False)
                vision_tower.requires_grad_(False)
                ...
                if "mm_vision_tower" in tunable_parts:
                    for name, param in model.named_parameters():
                        if "vision_tower" in name:
                            param.requires_grad_(True)
                        # if "occ_tower" in name:
                        #     param.requires_grad_(True)
                if "mm_language_model" in tunable_parts:
                    for name, param in model.named_parameters():
                        if "vision_tower" not in name and "mm_projector" not in name and "vision_resampler" not in name:
                            if ("model.layers.26" in name or "model.layers.0" in name or 
                                "model.layers.27" in name or "model.layers.1" in name or 
